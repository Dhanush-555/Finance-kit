...
import { Globe, Menu, Bell, LogOut, Ghost, EyeOff } from 'lucide-react';
import { motion } from 'framer-motion';
import { NotificationCenter } from '../notifications/NotificationCenter';

export const Topbar: React.FC<{ onMenuClick: () => void }> = ({ onMenuClick }) => {
  const { language, setLanguage, loans, chits, signOut, profile, isStealthMode, toggleStealthMode } = useFinance();
  ...
  return (
    <header className="h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-4 md:px-6 sticky top-0 z-10 shrink-0">
      <div className="flex items-center gap-3">
        <motion.button whileTap={{ scale: 0.9 }} onClick={onMenuClick} className="md:hidden p-2 text-slate-500 hover:bg-slate-100 rounded-lg transition-colors">
          <Menu size={22} />
        </motion.button>
      ...
      <div className="flex items-center gap-3 relative">
        <motion.button 
          ...
          className={`p-2 rounded-full transition-all flex items-center justify-center ${isStealthMode ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:bg-slate-100'}`}
        >
          {isStealthMode ? <EyeOff size={20} /> : <Ghost size={20} />}
        </motion.button>
